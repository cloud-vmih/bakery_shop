...
export const InventoryProvider = ({ children }: { children: ReactNode }) => {
  ...
  const summary: InventorySummary = {
    ...
    totalAvailable: inventory.reduce(
      (sum, item) => sum + (item.stockQuantity - item.reservedQuantity),
      0
    ),
    lastUpdated:
      inventory.length > 0
        ? new Date(
            Math.max(...inventory.map((i) => new Date(i.updatedAt).getTime()))
          ).toLocaleString()
        : null,
  };
  ...
  const getItemQuantity = useCallback(
    (itemId: number, branchId: number): number => {
      ...
      return Math.max(item.stockQuantity - item.reservedQuantity, 0);
    },
    [inventory]
  );
