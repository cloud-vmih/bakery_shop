 ...
 */
export const getWishlistByCustomerId = async (customerId: number): Promise<Item[]> => {
 ...
 */
export const addProductToWishlistDB = async (customerId: number, itemId: number) => {
  const repo = AppDataSource.getRepository(Wishlist);
 ...
 */
export const removeProductFromWishlistDB = async (customerId: number, itemId: number) => {
  const repo = AppDataSource.getRepository(Wishlist);
