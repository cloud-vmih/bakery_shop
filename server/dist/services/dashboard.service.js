...
const getDashboardData = async (filters) => {
    ...
    const orders = await (0, manageOrder_db_1.findOrdersInRangeWithDetails)(fromDate, toDate);
    if (orders.length === 0) {
        return {
            summary: {
                netRevenue: 0,
    ...
    const prevNetRevenue = await (0, manageOrder_db_1.getNetRevenueInRange)(prevFrom, prevTo);
