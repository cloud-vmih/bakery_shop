...
const registerUser = async (username, password, email, phoneNumber, fullName, dateOfBirth, avatarURL) => {
    try {
        ...
        if (await (0, db_account_1.findUserByEmail)(email))
        ...
        await (0, sendEmail_1.sendEmail)(user.email, "Verify your email", `
...
exports.googleService = {
    loginWithGoogle: async (idToken) => {
        ...
        if (social) {
            ...
            user = await (0, db_account_1.findUserByEmail)(email);
            if (!user) {
            ...
            await db_account_1.socialAuthRepo.linkSocialAccount(providerUserId, email, accountId);
        ...
        const token = jsonwebtoken_1.default.sign(jwtPayload, process.env.JWT_SECRET, { expiresIn: "15m" });
        const refresh_token = jsonwebtoken_1.default.sign(jwtPayload, process.env.JWT_SECRET, { expiresIn: "7d", });
...
exports.changePassword = {
    sendOTP: async (email) => {
      ...
      <a>${otp}</a>
    `;
            (0, sendEmail_1.sendEmail)(email, "Verify your OTP", html);
    ...
    resetPassword: async (newPassword, email) => {
        ...
        const user = await (0, db_account_1.findUserByEmail)(email);
        try {
            const hash = await bcryptjs_1.default.hash(newPassword, 10);
            await (0, db_account_1.updatePassword)(hash, user?.account?.id);
